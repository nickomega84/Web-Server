    ...
    while (1)
	{
        ...
        for (int i = 0; i < event_nmb; i++)
		{
			if (std::find(listen_sockets.begin(), listen_sockets.end(), events[i].data.fd) != listen_sockets.end()) // si registramos una lectura en uno de los listen_socket, tenemos una nueva conexión
			{
				...
				epoll_ctl_call(epollfd, events[i].data.fd, EPOLLIN | EPOLLOUT | EPOLLERR | EPOLLHUP); // los sockets de los clientes deberan estar atentos a leer y escribir (de acuerdo al subject) // si un mismo socket de cliente recibe varios eventos a la vez, se acumulan todos en epoll_event.events
            }
